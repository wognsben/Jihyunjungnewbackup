...
export const BlockRenderer = ({
  ...
  const firstMediaGroupIndex = groups.findIndex((group) => {
  if (group.type === 'image-slider') return true;

  const firstBlock = group.blocks[0];
  return firstBlock?.type === 'image';
});
  ...
  const shouldPrioritizeThisGroup =
    prioritizeFirstMedia && index === firstMediaGroupIndex;
  ...
  if (group.type === 'image-slider') {
    return (
      <ImageSliderBlock
        ...
        blocks={group.blocks}
        compact={compact}
        priority={shouldPrioritizeThisGroup}
      />
    );
  }
